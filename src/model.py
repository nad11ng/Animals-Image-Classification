...
def main():
    ...
    # Test function
    def test_forward_pass(model, batch_size, image_size, num_classes=10):
        ...
        print(f"Input dummy_images: {tuple(dummy_images.shape)} → Output dummy_images: {actual_shape}")

        assert actual_shape == expected_shape, (f"Expected output shape {expected_shape}, "f"but received {actual_shape}.")
        assert torch.isfinite(outputs).all(), ("Output contains NaN or infinite values.")
    ...
    with torch.inference_mode():
        ...
        for block_index, conv_block in enumerate(model.conv_features, start=1):
            x = conv_block(x)
    
            print(f"After convolution block {block_index}: {x.shape}")
